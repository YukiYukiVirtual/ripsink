...
class Setting
{
	...
	static int eyeRate = 30;
	static int eyeLength = 2;
}
...
public class Main extends JFrame implements WindowListener, ActionListener
{
	...
	void init_controlFrame()
	{
		...
		ripThreshold = new JTextArea(""+Setting.threshold);
		...
		delayTime = new JTextArea(""+Setting.delay);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(src == thresholdButton)
		{
			...
			{
				Setting.threshold = Integer.parseInt(text);
			}
		}
	...
	public static void main(String[] args)
	{
		for(String s:args)
		{
			try{
				...
				else if(key.equals("eyeRate"))
					Setting.eyeRate = Integer.parseInt(value);
				
				else if(key.equals("eyeLength"))
					Setting.eyeLength = Integer.parseInt(value);
		...
		System.out.println("==============================================");
		if(!new File(Setting.path).exists())
		{
			System.out.println(Setting.path+"というディレクトリが見つかりません");
...
class CharacterPanel extends JPanel implements Runnable
{
	...
	CharacterPanel()
	{
		mouth = new BufferedImage[Setting.mouthLength];
		eye = new BufferedImage[Setting.eyeLength];
		bgColor = new Color(Setting.bgColor);
		try
		{
			base = ImageIO.read(new File(Setting.path + "/base.png"));
		}catch(Exception e)
		{
			//e.printStackTrace();
			System.out.println("base.pngの読み込みに失敗");
		}
		
		int i=0;
		try
		{
			for(;i<mouth.length;i++)
			{
				mouth[i] = ImageIO.read(new File(Setting.path + "/mouth"+i+".png"));
			}
		}catch(Exception e)
		{
			//e.printStackTrace();
			if(i==0)
			{
				System.out.println("mouth0.pngが見つからなかったためこれ以上実行できません。オプション「mouthLength:0」を使うか、ファイル名を確認してください");
				System.exit(1);
			}
			System.out.println("mouth"+i+".pngの読み込みに失敗");
			System.out.println("以降を読み込めた画像で埋めます");
			for(;i<mouth.length;i++)
			{
				if(mouth[i]==null)
				{
		...
		try
		{
			for(;i<eye.length;i++)
			{
				eye[i] = ImageIO.read(new File(Setting.path + "/eye"+i+".png"));
			}
		}catch(Exception e)
		{
			//e.printStackTrace();
			if(i==0)
			{
				System.out.println("eye0.pngが見つからなかったためこれ以上実行できません。オプション「eyeLength:0」を使うか、ファイル名を確認してください");
				System.exit(1);
			}
			System.out.println("eye"+i+".pngの目の読み込みに失敗");
			System.out.println("以降を読み込めた画像で埋めます");
			for(;i<eye.length;i++)
			{
				if(eye[i]==null)
	...
	public void paint(Graphics g)
	{
		...
		if(Setting.mouthLength != 0)
			g.drawImage(mouth[mouthCount], 0, 0, getWidth(), getHeight(), null);
		if(Setting.eyeLength != 0)
			g.drawImage(eye[eyeCount], 0, 0, getWidth(), getHeight(), null);
		if(statusBar != null)
		{
			String str = String.format("<html>声の大きさ:%d %s<br>口の大きさ:%d/%d",
				vqueue[Setting.delay], (vqueue[Setting.delay] > Setting.threshold?"Over":""),
				mouthCount, mouth.length-1);
	...
	{
		Setting.delay = Math.max(0,Math.min(o,vqueue.length-1));
	}
	...
	{
		if(vqueue[Setting.delay] > Setting.threshold)// 声の大きさが閾値を超えている
		{
			// 口を開けようとする
		...
		if(frame % Setting.eyeRate == 0)// 目を閉じるかどうか
		{
			isClosing = true;
		}
		if(isClosing)
		{
			// 目を閉じようとする
			eyeCount++;
			if(eyeCount >= Setting.eyeLength-1)
			{
				isClosing = false;
				eyeCount = Setting.eyeLength-1;
			}
		}
		else
		{
	...
	public void run()
	{
		int ms = 1000/Setting.frameRate;
		int freqency = 8000;
		int bits = 16;
		AudioFormat format = new AudioFormat(freqency, bits, 1, true, false);
		Recorder rec = new Recorder(format, ms/4, Setting.deviceName);
		int count = 0;
		while(rip)
		{
			try
